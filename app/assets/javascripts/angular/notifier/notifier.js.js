...
app.factory("UiNotifier", function() {
  class UiNotifier {
    ...
    hide() {
      return $(".top-right div").css({transition: "opacity 0.3s"}).css({opacity: 0}).css({transition: ""});
    }

    render_error_array(arr, prefix_msg){
      if (prefix_msg == null) { prefix_msg = "Folgende Fehler sind aufgetreten: "; }
    ...
    errors_or_fallback(data, fallback) {
      if (fallback == null) { fallback = "Ein unbekannter Fehler is aufgetreten"; }
      if (data && data.errors) {
