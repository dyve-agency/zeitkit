...
app.controller("WorklogsController", ["$scope", "$q", "Worklog", "Client", "User", function($scope, $q, Worklog, Client, User){
  $scope.init = function() {
    $scope.hstep = 1;
    $scope.mstep = 10;
    $scope.clients = [];
    ...
    return $q.all(query).then(function(results){
      ...
      if (gon.client_id) {
        const getSelectedClient = () => _.select($scope.clients, c=> c.id === gon.client_id)[0];
